...
import android.os.Bundle; // Библиотека для работы с состояниями активностей
...
public class MainActivity extends AppCompatActivity implements NameListFragment.OnNameSelectedListener {
    ...
    private void loadFragment(Fragment fragment, boolean addToBackStack) {
        FragmentTransaction transaction = getSupportFragmentManager().beginTransaction(); // Начинаем транзакцию фрагмента
        transaction.replace(R.id.fragment_container, fragment); // Заменяем текущий фрагмент на новый
        if (addToBackStack) {
            transaction.addToBackStack(null); // Добавляем транзакцию в back stack, если это необходимо
        }
